...
class predicter:
    ...
    def load_model(self, from_local: bool = False):
        """
        loads model from either local repository or from hugging face pipeline
        from_local: bool
            loads model locally if true. else loads transformer pipeline
        """
        if from_local == True:
            model = AutoModelForSequenceClassification.from_pretrained(
                self.model_ckpt, num_labels=2
            )
            checkpoint = torch.load(self.load_path, map_location=torch.device("cpu"))
    ...
    def predict(self, text: str, from_local: bool = False) -> int:
        """
